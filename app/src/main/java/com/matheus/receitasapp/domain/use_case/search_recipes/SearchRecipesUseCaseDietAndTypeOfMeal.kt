...
class SearchRecipesUseCaseDietAndTypeOfMeal @Inject constructor(
    private val repository: RecipesRepository
) {

    operator fun invoke( query: String, diet: String, typeOfMeal: String ): Flow<Resource<List<Hit>>> = flow {

        try {
            emit( Resource.Loading<List<Hit>>() )
            val recipes = repository.searchRecipesByMealTypeAndDiet( query, diet, typeOfMeal )
