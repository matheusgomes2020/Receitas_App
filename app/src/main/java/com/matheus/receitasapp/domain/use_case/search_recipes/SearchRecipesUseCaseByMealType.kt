...
class SearchRecipesUseCaseByMealType @Inject constructor(
    private val repository: RecipesRepository
) {

    operator fun invoke( query: String, mealType: String ): Flow<Resource<List<Hit>>> = flow {

        try {
            emit( Resource.Loading<List<Hit>>() )
            val recipes = repository.searchRecipesByMealType( query, mealType )
